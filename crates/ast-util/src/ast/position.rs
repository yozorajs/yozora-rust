    ...
    loop {
        if let Some(frame) = stack.last_mut() {
            ...
            if let Node::Custom(custom) = &mut node {
                remove_positions_from_custom_data(&mut custom.data);
            }
...
fn remove_positions_from_custom_data(data: &mut Map<String, Value>) {
    for value in data.values_mut() {
        let Value::Array(items) = value else {
            continue;
        };
        for item in items {
            remove_position_from_json_node(item);
        }
    }
}

fn remove_position_from_json_node(value: &mut Value) {
    let Value::Object(node) = value else {
        return;
    };
    if !node.get("type").is_some_and(Value::is_string) {
        return;
    }
