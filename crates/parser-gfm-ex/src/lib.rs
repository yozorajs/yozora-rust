...
pub type GfmExParserProps = DefaultParserProps;
...
impl GfmExParser {
    pub fn new(mut props: GfmExParserProps) -> Self {
        props
...
impl Parser for GfmExParser {
    fn use_tokenizer(
        &mut self,
        tokenizer: AnyTokenizer,
        register_before_tokenizer: Option<&str>,
    ) -> &mut Self {
        GfmExParser::use_tokenizer(self, tokenizer, register_before_tokenizer)
    }
    ...
    fn replace_tokenizer(
        ...
        GfmExParser::replace_tokenizer(self, tokenizer, register_before_tokenizer)
    }

    fn unmount_tokenizer(&mut self, tokenizer_name: &str) -> &mut Self {
        GfmExParser::unmount_tokenizer(self, tokenizer_name)
    }

    fn use_fallback_tokenizer(&mut self, tokenizer: AnyFallbackTokenizer) -> &mut Self {
        GfmExParser::use_fallback_tokenizer(self, tokenizer)
    }

    fn set_default_parse_options(&mut self, options: Option<ParseOptions>) {
        GfmExParser::set_default_parse_options(self, options)
    }
    ...
    where
        C: Into<ParseContents<'a>>,
    {
        GfmExParser::parse(self, contents, options)
    }
}
