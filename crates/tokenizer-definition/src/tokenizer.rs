...
use yozora_character::{
    calc_escaped_string_from_node_points, calc_string_from_node_points, fold_case,
    is_ascii_control_character, is_whitespace_character, AsciiCodePoint, NodePoint,
    VirtualCodePoint,
};
...
#[derive(Debug, Clone)]
struct LinkLabelCollectingState {
    saturated: bool,
    node_points: Vec<NodePoint>,
    has_non_whitespace_character: bool,
}

#[derive(Debug, Clone)]
struct LinkDestinationCollectingState {
    saturated: bool,
    node_points: Vec<NodePoint>,
    has_open_angle_bracket: bool,
    open_parens_count: i32,
}

#[derive(Debug, Clone)]
struct LinkTitleCollectingState {
    saturated: bool,
    node_points: Vec<NodePoint>,
    wrap_symbol: Option<i32>,
}
...
struct TokenData {
    ...
    destination: Option<LinkDestinationCollectingState>,
    title: Option<LinkTitleCollectingState>,
    line_no_of_label: usize,
    line_no_of_destination: isize,
    line_no_of_title: isize,
}
...
impl MatchBlockHook for DefinitionMatchHook<'_> {
    ...
    fn eat_opener(
        ...
        if !data.label.saturated {
            let token =
                BlockToken::new("", DEFINITION_TYPE, calc_line_position(line)).with_data(data);
    ...
    fn eat_continuation_text(
        ...
        if data.destination.is_none() {
            ...
            let destination_result =
                eat_and_collect_link_destination(node_points, i, end_index, None);
        ...
        let title_result =
            eat_and_collect_link_title(node_points, i, end_index, data.title.clone());
    ...
    fn on_close(&mut self, token: &BlockToken) -> Option<OnCloseResult> {
        ...
        self.api.register_definition_identifier(&identifier);
        result
    }
}
    ...
    fn parse(&self, tokens: &[BlockToken]) -> Vec<Node> {
        ...
        for token in tokens {
            ...
            nodes.push(Node::Definition(Definition {
                ...
                identifier,
                label,
                url: self.api.format_url(&destination),
                title,
            }));
        }
...
fn resolve_label_and_identifier(label_points: &[NodePoint]) -> Option<(String, String)> {
...
fn calc_effective_position(token: &BlockToken, data: &TokenData) -> Option<Position> {
