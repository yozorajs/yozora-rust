use yozora_ast::{InlineMath, Node, INLINE_MATH_TYPE};
...
impl InlineTokenizer for InlineMathTokenizer {
    fn tokenize_inline(
        ...
        let tokens = r#match::match_inline_math_tokens(input, self.backtick_required)?;
        Some(parse::parse_inline_math_tokens(input, &tokens))
    }
...
#[derive(Debug, Clone)]
struct InlineMathTokenData {
    value: String,
}

impl EngineTokenizer for InlineMathTokenizer {
...
struct DelimiterEntry {
    ...
    data: InlineMathTokenData,
}

struct InlineMathMatchHook {
...
impl InlineMathMatchHook {
    fn new(api: &dyn EngineMatchInlinePhaseApi, backtick_required: bool) -> Self {
        ...
        let matched =
            r#match::match_inline_math_tokens(&source, backtick_required).unwrap_or_default();

        let mut delimiters = Vec::new();
        for token in matched {
            let InlineMathToken::InlineMath { interval, value } = token else {
                continue;
            };
            ...
            delimiters.push(DelimiterEntry {
                ...
                data: InlineMathTokenData { value },
            });
        }
    ...
    fn lookup_data(&self, delimiter: &TokenDelimiter) -> Option<&InlineMathTokenData> {
        self.delimiters
            .iter()
            .find(|x| {
...
impl MatchInlineHook for InlineMathMatchHook {
    fn reset(&mut self) {
        self.cursor = 0;
...
impl EngineInlineTokenizer for InlineMathTokenizer {
    fn create_match_hook<'a>(
        &'a self,
        api: &'a dyn EngineMatchInlinePhaseApi,
    ) -> Box<dyn MatchInlineHook + 'a> {
        Box::new(InlineMathMatchHook::new(api, self.backtick_required))
    }
...
mod tests {
    ...
    #[test]
    fn engine_match_should_find_inline_math_delimiter() {
        ...
        assert_eq!(delimiter.end_index, 3);
    }
