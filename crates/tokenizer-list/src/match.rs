...
#[derive(Debug, Clone)]
pub(crate) struct ListItemToken {
...
pub(crate) fn can_interrupt_paragraph(lines: &[&str]) -> bool {
...
pub(crate) fn match_list_token(lines: &[&str], enable_task_list_item: bool) -> Option<ListToken> {
    ...
    while consumed_lines < lines.len() {
        ...
        let (item, item_consumed, item_has_internal_blank) =
            consume_list_item(lines, consumed_lines, &marker, enable_task_list_item);
        if item_consumed == 0 {
            break;
        }
...
fn consume_list_item(
    ...
    let mut count_of_top_blank_line = if marker.content.is_empty() {
        1usize
    } else {
        0usize
    };
    ...
    let (status, first_line) = parse_task_status(
        logical_lines.first().map(String::as_str).unwrap_or(""),
        enable_task_list_item,
    );
...
fn parse_task_status(content: &str, enable_task_list_item: bool) -> (Option<TaskStatus>, String) {
