use yozora_ast::{Node, Point, Position, Table, TABLE_TYPE};
...
struct TokenData {
    ...
    latest: Option<r#match::TableToken>,
}
...
struct TableMatchHook;

impl MatchBlockHook for TableMatchHook {
    ...
    fn eat_opener(
        ...
        let token =
            BlockToken::new("", TABLE_TYPE, calc_line_position(line)).with_data(TokenData {
                lines: vec![source],
    ...
    fn eat_continuation_text(
        ...
        let matched = r#match::match_table_token(&refs);
...
impl ParseBlockHook for TableParseHook<'_> {
    fn parse(&self, tokens: &[BlockToken]) -> Vec<Node> {
        ...
        for token in tokens {
            ...
            let mut node = parse::parse_table_token(matched).node;
            if let Node::Table(Table { position, .. }) = &mut node {
